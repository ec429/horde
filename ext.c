...
int main(int argc, char **argv)
{
	...
	while(!errupt)
	{
		...
		if(h)
		{
			...
			{
				fprintf(stderr, "horde: %s[%d]: looking up '%s'\n", name, getpid(), h->data);
				char *ctype=mime_type(h->data);
				if(!ctype||!*ctype)
				{
					fprintf(stderr, "horde: %s[%d]: not found\n", name, getpid());
					if(ctype) free(ctype);
					ctype=strdup("application/octet-stream");
				}
				if(!ctype)
				{
					fprintf(stderr, "horde: %s[%d]: allocation failure (ctype): strdup: %s\n", name, getpid(), strerror(errno));
				...
				{
					fprintf(stderr, "horde: %s[%d]: found; is %s\n", name, getpid(), ctype);
